...
class XKlass_Live_Categories(Screen):
    ALLOW_SUSPEND = True
    ...
    def xmltvCheckData(self):
        ...
        filename = "xklass.sources.xml"
        sourcepath = os.path.join(filepath, filename)
        epgfilename = "xklass.{}.channels.xml".format(safeName)
                ...
                if self.list2:
                    ...
                    if playing:
                        if self.session.nav.getCurrentlyPlayingServiceReference().toString() != self.reference.toString() and cfg.livepreview.value is True:
                            ...
                            update_channel_icons_and_list()
                            self.session.openWithCallback(self.reload, liveplayer.XKlass_StreamPlayer, str(next_url), str(streamtype), stream_id)
                    else:
    ...
    def buildXMLTV(self):
        ...
        sourcefile = "/etc/epgimport/xklass.sources.xml"
        if not os.path.isfile(sourcefile) or os.stat(sourcefile).st_size == 0:
            with open(sourcefile, "w") as f:
                xml_str = '<?xml version="1.0" encoding="utf-8"?>\n'
                xml_str += '<sources>\n'
                xml_str += '<sourcecat sourcecatname="XKlass EPG">\n'
                xml_str += '</sourcecat>\n'
                xml_str += '</sources>\n'
                f.write(xml_str)
    ...
    def showPopupMenu(self):
        ...
        glob.ChoiceBoxDialog = self.session.instantiateDialog(channelmenu.XKlass_ChannelMenu, "live")
